...
class PlotAnalyseCanvas(FigureCanvas):
	...
	def plot(self):
		...
		ax_y.set_xlabel('I / \u03a3 I [%]')
		ax_x.set_ylabel('I / \u03a3 I [%]')
...
class MyMainWindow(QMainWindow):
	...
	def initUi(self):
		...
		global version
		self.setWindowTitle('Another low budget Beam Profiler Version {} ==== Special version: Fixed Gaussian order 1 ====='.format(version))
		self.show()
		self.center()
...
class FormWidget(QWidget):
	...
	def write_to_table(self):
		if self.m.last_rslt is None:
			...
			return None
		FWHMx = [self.FWHM(x) for x in self.m.last_rslt[:2]]
		FWHMy = [self.FWHM(y) for y in self.m.last_rslt[2:]]
